...
module.exports = function(robot) {
  ...
  const bozzcoinSummaries = function() {
    let output = "\n";
    ...
    for (let name of Array.from(sortedBozzcoinTracker)) {
      const bozzcoinsContributed = bozzcoinTracker[name];
      output += (name === "rurouni" ? `*${name}*` : name) + `: ${bozzcoinsContributed} :bozzcoin: contributed.\n`;
    }
    return output;
  };

  const earnRate = function(exerciseType) {
    switch (exerciseType) {
      case "cycle": return 20;
      case "pullups": return 10;
      case "pushups": case "situps": case "squats": case "lunges": return 1;
      case "run": return 50;
      case "racket steps": return 1 / 18;
      default: return 0;
    }
  };

  const verbToExerciseType = function(verb) {
    switch (verb) {
      case "ran": return "run";
  ...
  const updateBozziplier = function(username) {
    if (username === "rurouni") {
  ...
  const convertToBozzcoin = (reps, exerciseType) => Math.round(reps * earnRate(exerciseType) * robot.brain.get("bozziplier"));

  const convertToReps = (bozzcoin, exerciseType) => bozzcoin / earnRate(exerciseType);

  const cheatDayPrices = function(cheatType) {
    switch (cheatType) {
  ...
  robot.respond(new RegExp("set bozzcoins to (\\d+)", "i"), function(res) {
    if (res.message.user.name === "lockheed") {
      const bozzcoinBalance = Number.parseInt(res.match[1]);
      robot.brain.set("bozzcoinBalance", bozzcoinBalance);
      robot.brain.set("bozzcoinTracker", {});
      return res.send(`Bozzcoin balance reset to ${bozzcoinBalance} :bozzcoin:. Tracker reset.`);
    }
  });

  robot.respond(/bozzcoins\?/i, function(res) {
    const bozzcoinBalance = robot.brain.get('bozzcoinBalance');
    res.send(`*${bozzcoinBalance}* :bozzcoin:`);
    return res.send(bozzcoinSummaries());
  });

  robot.respond(new RegExp("did (-?\\d+) (pullups|pushups|situps|squats|lunges|racket steps)", "i"), function(res) {
    let repsByUser;
    const username = res.message.user.name;
    const repsDone = Number.parseInt(res.match[1]);
    ...
    return robot.brain.set("bozzcoinTracker", bozzcoinTracker);
  });

  robot.respond(new RegExp("(ran|cycled) (-?\\d+.?\\d*) ?km", "i"), function(res) {
    let distanceByUser;
    const username = res.message.user.name;
  ...
  return robot.respond(new RegExp("(prata|starbucks|macs) day", "i"), function(res) {
    ...
    if (newBozzcoinBalance < 0) {
      return res.send(`Not enough :bozzcoin: for a ${cheatType} day :(`);
    } else {
      robot.brain.set("bozzcoinBalance", newBozzcoinBalance);
      return res.send(`Happy *${cheatType} day*!! *${newBozzcoinBalance}* :bozzcoin: remaining.`);
    }
  });
};
